<template>
    <div class="mt-20" v-if="orders">
        ...
        <p class="text-center">Check status of orders</p>
            ...
            <div v-for="order in orders" :key="order.id">
                <div class="flex justify-between border-b-2 pb-1 mt-8">
                    ...
                    <div class="flex gap-4">
                        <button class="border-2 border-gray-500 rounded-sm p-2 py-1 px-2 text-gray-500" v-if="order.status == 'cancel'">Canceled</button>
                        ...
                        <router-link :to="{name: 'orderDetail', params: { id: order.id }}" class="bg-blue-400 hover:bg-blue-500 px-4 flex items-center justify-center text-white rounded-sm">
                                Detail
                        </router-link>
                    </div>
                </div>
    ...
    setup() {
        ...
        onMounted(async () => {
            try {
                ...
                let order = await axios.get("/orders/confirmed", {
                    headers: {
